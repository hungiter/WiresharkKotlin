...
import com.example.ptnet.models.MatchList.Rule
...
class CtrlPermissions {
    ...
    fun add(package_name: String?, tp: ConsentType?) {
        mRules[package_name] = Rule(package_name, tp)
        save()
    }

    fun remove(package_name: String?) {
    ...
    fun getConsent(package_name: String?): ConsentType? {
        val rule = mRules[package_name] ?: return ConsentType.UNSPECIFIED
        return rule.consent
    }
}