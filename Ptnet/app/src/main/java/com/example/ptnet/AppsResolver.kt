...
import com.example.ptnet.Utils.getPackageInfo
import com.example.ptnet.Utils.getPackageUid
import com.example.ptnet.models.AppDescriptor
import com.example.ptnet.interfaces.DrawableLoader
import com.example.ptnet.services.CaptureService
import java.lang.reflect.InvocationTargetException
import java.lang.reflect.Method


class AppsResolver(private val mContext: Context) {
    private val mApps: SparseArray<AppDescriptor?> = SparseArray<AppDescriptor?>()
    ...
    private fun initVirtualApps() {
        ...
        val unknownIconLoader =
            DrawableLoader { ContextCompat.getDrawable(mContext, android.R.drawable.ic_menu_help) }
        ...
        mApps.put(
            Utils.UID_UNKNOWN, AppDescriptor(
                mContext.getString(R.string.unknown_app),
        ...
        mApps.put(
            1001, AppDescriptor(
                mContext.getString(R.string.phone_app),
    ...
    fun getAppByUid(uid: Int, pm_flags: Int): AppDescriptor? {
        var app: AppDescriptor? = mApps[uid]
        ...
        if (!mFallbackToGlobalResolution && CaptureService().isCapturingAsRoot()) {
            try {
                ...
                if (pinfo != null) app = AppDescriptor(mPm, pinfo)
